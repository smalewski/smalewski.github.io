(function ghostCursor() {
  ...
  function addParticle(x, y) {
    var particle = new Particle(cursorImage);
    particle.init(x, y);
    ...
    this.update = function () {
      ...
      if (this.lifeSpan <= this.fadePoint) {
        context.globalAlpha = Math.max(
          (this.lifeSpan - 1) / (this.fadePoint - 1),
          0
        );
      }

      context.drawImage(srcImage, this.position.x, this.position.y);
    };
  }
